...
HIGH_RES_SETTINGS = {
    'width': 2400,  # Ширина для 600 DPI при 4 дюймах
    'height': 3200,  # Высота для 600 DPI при 5.33 дюймах
    'scale': 8,      # Масштаб для увеличения разрешения
    'format': 'png',
    'dpi': 600
}

# Установка формата по умолчанию для plotly
    ...
    try:
        with zipfile.ZipFile(zip_path, 'w') as zipf:
            ...
            fig1.add_hline(
                ...
                annotation_font=dict(size=24, color='red')
            )
            
            fig1_bytes, fig1_name = save_high_res_figure(
                fig1, 
                "01_Experimental_Data.png",
                "Experimental Data",
                include_title=True
            )
            
            if fig1_bytes:
                zipf.writestr(fig1_name, fig1_bytes)
                all_files.append(fig1_name)
            ...
            fig2.add_trace(go.Scatter(
                ...
                line=dict(
                    ...
                    width=PUBLICATION_STYLE['line_width'] * 2
                ),
                name=f'Linear fit: R² = {results["method1"]["r_squared"]:.4f}',
                showlegend=True
            ))
            
            fig2_bytes, fig2_name = save_high_res_figure(
                fig2,
                "02_Method1_lnKw_vs_1000_T.png",
                "Method 1: ln(Kw) vs 1000/T",
                include_title=True
            )
            
            if fig2_bytes:
                zipf.writestr(fig2_name, fig2_bytes)
                all_files.append(fig2_name)
            
            # 3. Method 2: Profile Fitting with Residuals
            ...
            fig3_bytes, fig3_name = save_high_res_figure(
                fig3,
                "03_Method2_Fitting_with_Residuals.png",
                "Method 2: Profile Fitting with Residuals",
                include_title=True
            )
            
            if fig3_bytes:
                zipf.writestr(fig3_name, fig3_bytes)
                all_files.append(fig3_name)
            ...
            if show_method2_comparison:
                ...
                fig4.add_trace(go.Scatter(
                    ...
                    name='Method 2' if not show_method1_comparison else f'Method 2: ΔH = {results["method2"]["dH"]/1000:.1f} kJ/mol',
                    showlegend=True
                ))
            ...
            fig4_bytes, fig4_name = save_high_res_figure(
                fig4,
                "04_Comparison_of_Methods.png",
                "Comparison of Methods",
                include_title=True
            )
            
            if fig4_bytes:
                zipf.writestr(fig4_name, fig4_bytes)
                all_files.append(fig4_name)
            ...
            fig5_bytes, fig5_name = save_high_res_figure(
                fig5,
                "05_Temperature_Dependence_of_Kw.png",
                "Temperature Dependence of Kw",
                include_title=True
            )
            
            if fig5_bytes:
                zipf.writestr(fig5_name, fig5_bytes)
                all_files.append(fig5_name)
            
            # 6. 3D Surface Plot
            fig6 = create_3d_surface(results, colors, palette_design, use_log_pH2O)
            if fig6:
                # Обновляем размер для высокого разрешения
                fig6.update_layout(
                    width=HIGH_RES_SETTINGS['width'],
                    height=HIGH_RES_SETTINGS['height'],
                    scene=dict(
                        camera=dict(
                            eye=dict(x=1.5, y=1.5, z=1.5)  # Улучшаем обзор для сохранения
                        )
                    )
                )
                
                fig6_bytes, fig6_name = save_high_res_figure(
                    fig6,
                    "06_3D_Surface.png",
                    "3D Surface: [OH] = f(T, pH₂O)",
                    include_title=True
                )
                
                if fig6_bytes:
                    zipf.writestr(fig6_name, fig6_bytes)
                    all_files.append(fig6_name)
            
            # 7. README файл с параметрами
            readme_content = f"""High Resolution Figures (600 DPI)
...
Method 1: {colors['method1']}
Method 2: {colors['method2']}
3D Palette: {palette_design}

EXPORT SETTINGS:
Resolution: {HIGH_RES_SETTINGS['dpi']} DPI
Format: {HIGH_RES_SETTINGS['format'].upper()}
Dimensions: {HIGH_RES_SETTINGS['width']} × {HIGH_RES_SETTINGS['height']} pixels

FILE LIST:
{chr(10).join(all_files)}

This archive contains publication-quality figures suitable for scientific journals.
All figures are in PNG format with 600 DPI resolution."""
        ...
        return zip_bytes, len(all_files)
...
def get_zip_download_link(zip_bytes, filename="high_res_figures.zip"):
    """Generate download link for ZIP file"""
    b64 = base64.b64encode(zip_bytes).decode()
    href = f'<a href="data:application/zip;base64,{b64}" download="{filename}">📥 Download All Figures (ZIP, 600 DPI)</a>'
    return href
...
if n_total_points > 0:
    ...
    if results is None:
        ...
        with col_exp3:
            # Кнопка для генерации и скачивания ZIP
            if st.button("🖼️ Generate High-Res Figures", use_container_width=True):
                with st.spinner("Generating high-resolution figures (600 DPI)..."):
                    zip_bytes, num_files = save_all_figures_to_zip(
                    ...
                    if zip_bytes:
                        st.success(f"Generated {num_files} high-resolution figures!")
                        
                        # Создаем кнопку скачивания
                        b64 = base64.b64encode(zip_bytes).decode()
                        href = f'<a href="data:application/zip;base64,{b64}" download="hydration_thermo_figures_600dpi.zip">📥 Download All Figures (ZIP, 600 DPI)</a>'
                        st.markdown(href, unsafe_allow_html=True)
                    else:
                        st.error("Failed to generate figures. Please check the console for errors.")
